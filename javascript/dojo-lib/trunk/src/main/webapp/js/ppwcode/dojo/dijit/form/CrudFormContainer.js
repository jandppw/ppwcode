dojo.provide("ppwcode.dojo.dijit.form.CrudFormContainer");
...
dojo.declare(
	"ppwcode.dojo.dijit.form.CrudFormContainer",
			...
			for (var i = 0; i < children.length; i++) {
				...
				if (dojo.attr(contentpane.domNode, 'isEmptyPane') == "true") {
					...
					dojo.forEach(list, function(theform) {
							...
							this._constructorNameMap[ppwcode.dojo.util.JavaScriptHelpers.getConstructorFunctionName(mapproperties.form.getConstructorFunction())] = dojo.attr(theform, "id");
						}, this);
				}
			}
...
//		},
		...
		displayItem: function(/*Object*/obj) {
			this.resetCurrentForm();
			var theformid = this._constructorNameMap[ppwcode.dojo.util.JavaScriptHelpers.getConstructorFunctionName(obj.constructor)];
		...
		createObject: function(/*String || Object*/ctornameorproto) {
			...
			if (dojo.isString(ctornameorproto)) {
				...
				formid = this._constructorNameMap[ppwcode.dojo.util.JavaScriptHelpers.getConstructorFunctionName(ctornameorproto.constructor)];
			}
			if (formid && this._formIdMap[formid]) {
