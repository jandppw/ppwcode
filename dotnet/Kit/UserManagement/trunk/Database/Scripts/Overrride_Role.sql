...
GO
if object_id('[dbo].[P_Role_D_Override]') is not null
  DROP PROCEDURE [dbo].[P_Role_D_Override];
GO  
CREATE PROCEDURE [dbo].[P_Role_D_Override](
    @Context xml,
    @RoleID int,
...
begin
    ...
    begin try
      ...
      delete from dbo.RolePermission where RolePermission.RoleID = @RoleID;
      
      -- Execute the CRUD functionality.   
      exec @err = dbo.P_Role_D
        @Context,
        @RoleID,
        @errorinfo OUTPUT;
